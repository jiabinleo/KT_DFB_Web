    ...
    _.prototype.registerEditableShape = function(surface) {
        ...
        setListener(surface, 'mouseMove', function(position) {
            ...
            if(!surface._editMode) {
                _self._tooltip.showAt(position, "点击编辑");
            }
        });
...
//                                lineWidth : Math.min(this.strokeWidth || 4.0, context._aliasedLineWidthRange[1])
    ...
    var defaultBillboard = {
        iconUrl: "modules/map/drawler/img/dragIcon.png",
        shiftX: 0,
        shiftY: 0
    }
    ...
    var dragHalfBillboard = {
        iconUrl: "modules/map/drawler/img/dragIconLight.png",
        shiftX: 0,
        shiftY: 0
    }
    ...
    _.prototype.startDrawingMarker = function(options) {
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            if(position != null) {
                ...
                if (cartesian) {
                    tooltip.showAt(position, "<p>点击添加标记. 位置: </p>" + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
                } else {
    ...
    _.prototype.startDrawingPolyshape = function(isPolygon, options) {
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            if(position != null) {
                if(positions.length == 0) {
                    tooltip.showAt(position, "<p>点击添加第一个点</p>");
                } else {
                    var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                    if (cartesian) {
                        positions.pop();
                        ...
                        // show tooltip
                        tooltip.showAt(position, "<p>点击添加新一个点 (" + positions.length + ")</p>" + (positions.length > minPoints ? "<p>双击结束绘图</p>" : ""));
                    }
                }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
    ...
    _.prototype.startDrawingExtent = function(options) {
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            if(position != null) {
                if(extent == null) {
                    tooltip.showAt(position, "<p>点击开始绘制矩形</p>");
                } else {
                    var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                    if (cartesian) {
                        var value = getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian));
                        updateExtent(value);
                        tooltip.showAt(position, "<p>拖动改变矩形范围</p><p>再次点击结束绘制</p>");
                    }
                }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
                ...
                if(editMode) {
                    ...
                    if(this._markers == null) {
                        ...
                        function onEdited() {
                            ...
                            drawCircleObjext["width"] = circle.getRadius()/1000;
                            console.debug("修改后drawCircleObjext = "+JSON.stringify(drawCircleObjext))
                        }
                        var handleMarkerChanges = {
                            dragHandlers: {
                                onDrag: function(index, position) {
