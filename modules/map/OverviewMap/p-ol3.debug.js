...
P.Plot.DoubleCurve = function(points){
    goog.base(this, []);
    this.type = P.PlotTypes.DoubleCurve;
    this.t = 0.3;
    this.setPoints(points);
};

goog.inherits(P.Plot.DoubleCurve, ol.geom.MultiLineString);
goog.mixin(P.Plot.DoubleCurve.prototype, P.Plot.prototype);

P.Plot.DoubleCurve.prototype.generate = function(){
    var count = this.getPointCount();
    if(count < 2) {
        return;
    }
    if(count == 2) {
        this.setCoordinates(this.Doublelinecoords(this.points));
    } else {
        this.setCoordinates(this.Doublelinecoords(P.PlotUtils.getCurvePoints(this.t, this.points)));
    }
};
P.Plot.DoubleCurve.prototype.Doublelinecoords = function(points){
    var proto=getopenlayerpoltmap();
   var resolution= proto.map.getView().getResolution();
    var lineinterval=resolution*proto.doubleinterval;
    var newptarray=[];
    var array1=[];
    var array2=[];
    for(var i=0;i<points.length-1;i++)
    {
       var angle= P.PlotUtils.getAzimuth(points[i],points[i+1])
        if(isNaN(angle))
          continue;
        array1.push([ points[i][0]-lineinterval*Math.sin(angle),points[i][1]+lineinterval*Math.cos(angle)]);

        array2.push([points[i][0]+lineinterval*Math.sin(angle),points[i][1]-lineinterval*Math.cos(angle)]);
        if(i==points.length-2)
        {
            array1.push([ points[i+1][0]-lineinterval*Math.sin(angle),points[i+1][1]+lineinterval*Math.cos(angle)]);

            array2.push([points[i+1][0]+lineinterval*Math.sin(angle),points[i+1][1]-lineinterval*Math.cos(angle)]);
        }

    }
    newptarray.push(array1,array2)
    return newptarray;
};
...
P.Plot.DoublePolyline = function(points){
    goog.base(this, []);
    this.type = P.PlotTypes.DOUBLEPOLYLINE;
    this.setPoints(points);
};

goog.inherits(P.Plot.DoublePolyline, ol.geom.MultiLineString);
goog.mixin(P.Plot.DoublePolyline.prototype, P.Plot.prototype);

P.Plot.DoublePolyline.prototype.generate = function(){
    var count = this.getPointCount();
    if(count < 2) {
        return;
    }
...
P.Plot.DoublePolyline.prototype.Doublelinecoords = function(points){
    var proto=getopenlayerpoltmap();
    var resolution= proto.map.getView().getResolution();
    var lineinterval=resolution*proto.doubleinterval;
    var newptarray=[];
...
P.PlotFactory.createPlot = function(type, points){
    switch(type){
        ...
        case P.PlotTypes.DOUBLE_CURVE:
            return new P.Plot.DoubleCurve(points);
        ...
        case P.PlotTypes.DOUBLEPOLYLINE:
            return new P.Plot.DoublePolyline(points);
    }
    return null;
}
