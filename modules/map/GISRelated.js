...
var latLons = "";
// 多边形
function drawPolygon(callbacks) {
...
//设置落区id
function setFallId(id) {
    for (var i = 0; i < primitiveGrahpics.length; i++) {
        if (primitiveGrahpics[i].id == "defaultid")
            primitiveGrahpics[i].id = id;
    }
    var primitives = gis.scene.primitives;
...
//改变落区的颜色
function changeFallColor(id,color) {
    //var color=   new Cesium.Color(1,1,0,0.3);
    var primitives = gis.scene.primitives;
    var length = primitives.length;
    for (var j = 0; j < length; j++) {
        var p = primitives.get(j);
        if (p.id == id)
        {
            p.material=Cesium.Material.fromType("Color", {
                color: new Cesium.Color(color[0]/255,color[1]/255,color[2]/255,color[3]),
                //rimColor: new Cesium.Color(0, 0, 0, 0.4),
                //width: 0.3
            })
            //p.material=color;
...
function clearDrawing() {
    for (var i = 0; i < primitiveGrahpics.length; i++) {
        ...
        for (var j = 0; j < length; j++) {
            var p = primitives.get(j);
            if(p==null)continue;
            if (p.id == primitiveGrahpics[i].id) {
                primitives.remove(p);
            }
        }
    }
...
//通过id清除落区
function clearDrawingfallByid(id)
{
    for (var i = 0; i < primitiveGrahpics.length; i++) {
        if(primitiveGrahpics[i].id==id)
            primitiveGrahpics.remove(primitiveGrahpics[i]);
    }
    var primitives =  gis.scene.primitives;
    var length = primitives.length;
    for (var j = 0; j < length;j++) {
...
function addPolygoShape(coordinates, id) {
    ...
    var polygonGrahpic = gis.viewer.entities.add({
        ...
        polygon: {
            ...
            //height: 0,
            material:color,// Cesium.Color.YELLOW.withAlpha(0.3),
            outline: true,
...
//改变行政区域的颜色
function changePolyColor(id, colour) {
    var color = new Cesium.Color(1, 1, 0, 0.3);
    if (colour != null)
        color = new Cesium.Color(colour[0] / 255, colour[1] / 255, colour[2] / 255, colour[3]);
 ...
 *回到正北角度
 */
function backToDefaultangle() {
    if(gis.scene.mode==Cesium.SceneMode.SCENE2D)
      return;
    gis.camera.flyTo({
        destination :gis.camera.position,
        duration: 1
    });
}
...
function changeImageLayer(layerIdx) {
    ...
    if (1 == layerIdx) {
        gis.imageLayer.show=true;
        gis.epLayer.show=false;
        ...
        gis.imageLayer.show=false;
        gis.epLayer.show=true;
    } else {
        return;
    }
