...
//****************************** 配置信息 ******************************************************************************
    ...
    //影像图层
    gis.layer_image = {
        url: sysConfig.imageMapurl,
        layer: sysConfig.imageMapname
    };
    //行政区划图层
    gis.layer_region = {
        url: sysConfig.regionMapurl,
        layer: sysConfig.regionMapname
    };
    //高程图层
    gis.layer_dem = sysConfig.DEMUrl;
    //高程最大等级，决定查询地形高度精细度
    gis.terrainMaxLevel = 15;
    ...
    gis.defaultCameraView =sysConfig.defaultCameraView;
    //文字图形反锯齿
    var isFxaaOn = false;
    //最小缩放距离
     ...
     */
        //js文件路径根目录
    var jsUrlPrefix = gis.jsUrlPrefix = "modules/map/";
    //图片路径根目录
    gis.imgUrlPrefix = "modules/map/img/";
    //CSS文件根目录
    var cssUrlPrefix = "modules/map/css/";
    //地图容器DIV ID
...
//***************************** 配置完成 *******************************************************************************
    ...
    var viewer = gis.viewer = new Cesium.Viewer(mapContainerID, {
            ...
            // terrainExaggeration:2 //地形夸张
        });

    //viewer.scene.globe.maximumScreenSpaceError = 1.75;
    viewer.scene.globe.baseColor= new Cesium.Color(11/255, 23/255, 56/255, 1);
    ...
    //影像图层
    gis.imageLayer = new Cesium.ImageryLayer(new Cesium.WebMapServiceImageryProvider({
        url: gis.layer_image.url,
        layers: gis.layer_image.layer
    }), {
        brightness: 1,
        hue: 0,
        contrast: 1,
    ...
    //电子图层
    gis.epLayer = new Cesium.ImageryLayer(new Cesium.WebMapServiceImageryProvider({
        url: gis.layer_region.url,
        layers: gis.layer_region.layer,
        parameters: {
            format: "image/png"
        }
    }), {
    ...
    viewer.imageryLayers.add(gis.imageLayer);
