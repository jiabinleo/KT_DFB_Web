var retrieval = {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < result.length; i++) {
                    selectconten += "<option value='" + result[i].id + "'>" + result[i].name + "</option>";
    ...
    pointOnClick: function (e, obj) {
        if (retrieval.mapinfoHtml != null) {
            ...
            var detail = "<div id='" + obj.id + "' class='popout-div'>" + retrieval.mapinfoHtml + "</div>";
            var $ele = null,
                $ele = $(detail);
            // $ele.appendTo("body");
            $("#map-popout").html($ele)
        ...
        //    调请求数据的接口
            ...
            getTemplateHtml("modules/retrieval/mapinfoTemplate.html", function (content) {
...
//获取话题数据的数量
        var number_of_items = $('#retri-data').children().size();
//计算页数
...
//生成分页->上一页
        var page_info = '<li><a class="previous_link" href="javascript:retrieval.previousPage();">«</a></li>';
        var current_link = 0;
//生成分页->页数
        while (number_of_pages > current_link) {
            ...
            page_info += '<li><a class="page_link" href="javascript:retrieval.go_to_page(' + current_link + ')" longdesc="' + current_link + '">' + (current_link + 1) + '</a></li>';
            current_link++;
        }
//生成分页->下一页
        page_info += '<li><a class="next_link" href="javascript:retrieval.nextPage();">»</a></li>';
//加载分页
        $('.pagination').html(page_info);

//激活第一页，使得显示第一页
        $('#retrieve-page li').eq(1).addClass('active');
//隐藏该对象下面的所有子元素
        $('#retri-data').children().css('display', 'none');
//显示第n（show_per_page）元素
        $('#retri-data').children().slice(0, show_per_page).css('display', 'block');
    },
//    跳转某一页
...
//隐藏所有子div元素的内容,显示具体片数据，如显示25~30
        $('#retri-data').children().css('display', 'none').slice(start_from, end_on).css('display', 'block');
//每页显示的数目
        var show_per_page = 10;
...
//生成分页->前2页
            while (page_num > i) {
                page_info += '<li><a class="page_link" href="javascript:retrieval.go_to_page(' + i + ')" longdesc="' + i + '">' + (i + 1) + '</a></li>';
                i++;
            }
//生成分页->当前页
            page_info += '<li><a class="page_link" href="javascript:retrieval.go_to_page(' + page_num + ')" longdesc="' + page_num + '">' + (page_num + 1) + '</a></li>';
//生成分页->后2页
            while (p < j) {
                if (p == number_of_pages) {
                    break;
                }
                page_info += '<li><a class="page_link" href="javascript:retrieval.go_to_page(' + (p + 1) + ')" longdesc="' + (p + 1) + '">' + (p + 2) + '</a></li>';
                p++;
            }
//生成分页->下一页
            page_info += '<li><a class="next_link" href="javascript:retrieval.nextPage();">»</a></li>';
...
//当前页-1
    ...
    nextPage: function () {
        //当前页+1
        new_page = parseInt($('#current_page').val()) + 1;
        if(new_page < $("#retrieve-page li").eq(-2).text())
            retrieval.go_to_page(new_page);
    },
//    地图上的点位点击弹出弹框获取数据填充
